...
- We provide a development dataset; the evaluation will use an undisclosed dataset of similar same size computed with the same underlying LLM model.
...
- Container specifications: 8 virtual CPUs, 24 GB of RAM, the dataset will be mounted read-only into the container.
- Wall clock time for the entire task: 8 hours.
- Dataset: NQ (Natural questions) dataset (from BEIR, https://github.com/beir-cellar/beir) embedded with SPLADE-v3, around 2.68M documents, around 30K dimension.
...
- We provide a development dataset; the evaluation will use run on the same dataset vectors with queries that are computed with the same underlying model.
...
- See [https://huggingface.co/datasets/sadit/SISAP2026](https://huggingface.co/datasets/sadit/SISAP2026) for more details 
...
You can find more detailed information, data access, and registration at the SISAP Indexing Challenge website [https://sisap-challenges.github.io/2026/](https://sisap-challenges.github.io/2026/)
...
Write an email sisap-2026-indexing-challenge@googlegroups.com to contact any of the organizers.
